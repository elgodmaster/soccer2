...
<?php $form=$this->beginWidget('bootstrap.widgets.TbActiveForm', array(
	...
	for($i=0; $i<$numeroPartidos; $i++) //START FOR
	{
		...
		<?php 
		if($matchGames[$i]->GROUP != $switchVar){
		echo "<legend><h3>JORNADA ".$matchGames[$i]->GROUP ."</h3></legend>";
		$switchVar = $matchGames[$i]->GROUP;
		}
		?>
		...
		<?php 	
				...
				echo "<td> Cancha <br />";
					echo $form->dropDownList($matchGames[$i],"[$i]PLAY_GROUND_ID",CHtml::listData($playGround::model()->findAll(),'ID','NAME'));
				echo "</td>";
		?>
