 ...
 * https://github.com/blueimp/jQuery-File-Uploa
 *
...
class UploadHandler
{
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $fileName = str_replace('.', '_', 'DOCUMENT_'.$name);
        $fileType_ = str_replace('.', '_', 'DOCUMENT_TYPE_'.$name);
        
        $file->nombre = $_POST[$fileName];
        $file->tipoDocumento = $_POST[$fileType_];
        $file->propietario = $_POST['ownerId'];
    ...
    public function get($print_response = true) {
        ...
        if ($file_name) {
        	...
        	//$playerID = 1;// $session['nx_playerid'];
        	
        	$NX_object =  $session['_nx_object'];//Player::model()->findByPk($playerID);
        	
        	unset($session['_nx_object']);
		
        	$session->close();	
        	
        	        	
        	$documentsResult =  $NX_object->documents;//$documentPlayer->search()->getData();
        	
        	$indexFile=0;

        	foreach ($documentsResult as $documentData){
        		
        		
        		$prefixURL = '';//'/soccer2/files/';
        		$prefixDelURL=''; ///soccer2/index.php?r=player/deleteDocument&file=';
        		$fileToFront = new stdClass();
        		$fileToFront->tipoDocumento = $documentData->ID_DOCUMENT;
        		$fileToFront->name = $documentData->NAME;
        		$fileToFront->nombre = $documentData->DESCRIPTION;
        		$fileToFront->propietario = $documentData->getOwnerId();
        		$fileToFront->url = $documentData->PATH;
        		$fileToFront->size = $documentData->SIZE;
        		$fileToFront->type = $documentData->TYPE;
        		$fileToFront->thumbnail_url = $documentData->THUMBNAIL;
        		$fileToFront->delete_url = $documentData->DELURL;
        		$fileToFront->delete_type = $documentData->DELTYPE;
        		
        		$info[$indexFile++] = $fileToFront; 
 ...
 */
     ...
     * @return string
     */
    public function getFileContextPath($url){
    	
    	list($protocol, $host, $port, $mypath) = preg_split("/\//",$url,4);
     ...
     * @return string
     */
    public function getFileContextPathDel($url){
    	 
    	list($protocol, $myFile) = preg_split("/=/", $url,2);
    	
    	 
    	return '/soccer2/index.php?r=player/deleteDocument&file='.$myFile;
